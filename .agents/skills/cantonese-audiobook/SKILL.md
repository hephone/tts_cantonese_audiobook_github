...
- 不要把多个长章节同时提交给 `edge-tts`。批量时最多 2–3 个并发，并为每个子进程设置约 180 秒超时；超时只标记该文件，不能把整批误报为完成。
- 即使命令返回超时，也可能已经写出完整 MP3。重试前必须先用 `ffprobe` 检查；只有不存在、大小异常或不可解析时才重新生成，避免重复请求。
- 长章节优先按自然段拆成 `NNN-001-edge.txt`、`NNN-002-edge.txt` 等小段分别合成，再用 `ffmpeg -f concat -c copy` 合并。合并后再次 `ffprobe`，分段文件只作为中间产物。
- 旧脚本入口可能带有失效的虚拟环境 shebang；本项目统一使用 `.venv/bin/python -m edge_tts`，不要直接依赖 `.venv/bin/edge-tts`。

#### 命名与目录规则

- `Audio/` 生成期间可以暂存分段、重试和旧版本文件，但最终交付前必须只保留一套统一命名的正式 MP3：`001-recommendation.mp3`、`002.mp3` … `033.mp3`（按实际单元数量调整）。
- 中文标题文件、`*-edge.mp3`、`NNN-001-edge.mp3` 和试读文件都不能直接加入播放清单；它们属于旧版或中间产物。
- 播放清单必须使用相对路径，并且每个正式单元恰好出现一次。生成前检查文件名集合、顺序和数量，避免把分段或重复文件串入播放。
...
确认 MP3 存在、非空、可被 ffprobe 读取、时长与稿件体量合理，并向用户报告实际输出路径和试听结果。完整书籍验收后才在 `Audio/` 内创建只含最终章节文件名的 UTF-8 M3U 播放列表。清理备份目录和中间 MP3 前，必须先确认播放清单已改指向 `Audio/`，并逐条验证清单路径存在且可被 `ffprobe` 读取。
